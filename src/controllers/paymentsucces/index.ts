...
import { addEditPaymentSuccessSchema } from "../../validation/paymentSuccess";
...
export const addEditPaymentMessage = async (req, res) => {
    ...
    try {
        const { error, value } = addEditPaymentSuccessSchema.validate(req.body)
        if (error) return res.status(501).json(new apiResponse(501, error?.details[0]?.message, {}, {}))

        const response = await paymentMessageSuccessModel.findOneAndUpdate({ isDeleted: false }, value, { new: true, upsert: true })
        return res.status(200).json(new apiResponse(200, responseMessage?.updateDataSuccess("payment success"), response, {}))
    } catch (error) {
        console.log(error);
...
export const getPaymentMessages = async (req, res) => {
    ...
    try {
        const response = await paymentMessageSuccessModel.findOne({ isDeleted: false })
